...
export const buyProducts = (data) => new Promise(async (resolve, reject) => {
    try {
        ...
        const response = await Promise.all([
            db.Bill.create({ total: parseInt(data?.total), uid: data.uid, status: 'Pending',type :data.type, id: bid, coupon: data.coupon + '', isChecked: false }),
...
export const importProduct = (pid, payload, uid) => new Promise(async (resolve, reject) => {
    try {
        const bid = genId()
        const bulkData = [{pid, bid, quantity: payload?.quantity||0 , preQuantity: payload?.preQuantity || 0, price: payload?.price||0 }]
        const response = await db.Product.update({quantity: payload.quantity}, { where: { id: pid } })
        const res = await Promise.all([
            db.Billimport.create({pid,uid: uid, status: 'Pending', id: bid, isChecked: false}),
...
export const getImportBill = ({ limit, page, order, name }) => new Promise(async (resolve, reject) => {
    try {
        const offset = !page || +page <= 1 ? 0 : Number(page)
        const lt = Number(limit) || 20
        const queries = {}
